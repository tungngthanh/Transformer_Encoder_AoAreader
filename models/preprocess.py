...
#        train_stories, valid_stories, test_stories = Parallel(n_jobs=1)(delayed(get_stories)(story_lines)
#                                                          for story_lines in [tlines, vlines, telines])
    with open(train_filename, 'r') as tf:
        train_stories=get_stories(tf.readlines())
    print("finish making train_stories")
    with open(valid_filename, 'r') as vf:
        valid_stories=get_stories(vf.readlines())
    print("finish making dev stories")
    with open(test_filename, 'r') as tef:
        test_stories = get_stories(tef.readlines())
    print("finish making test stories")
