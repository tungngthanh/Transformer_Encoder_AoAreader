...
class AoAReader(nn.Module):
    ...
    def __init__(self, vocab_dict, dropout_rate, embed_dim, hidden_dim, bidirectional=True):
        ...
        self.transformer = Encoder(embed_dim, hidden_dim, 1, 1, hidden_dim, hidden_dim,
                 64, max_length=2000, input_dropout=0.0, layer_dropout=0.0,
    ...
    def forward(self, docs_input, docs_len, doc_mask,
        ...
        if candidates is not None:
            ...
            for i, cands in enumerate(candidates):
                ...
                for j, candidate in enumerate(cands):
                    ...
                    pb.append(torch.sum(torch.masked_select(s[i].squeeze(), pointer),0, keepdim=True))
        ...
        if answers is not None:
            ...
            for i, answer in enumerate(answers):
                ...
                this_prob = torch.sum(torch.masked_select(s[i].squeeze(), pointer),0,keepdim=True)
